...
public class viewOverallReport extends HttpServlet {
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ArrayList<String[]> billedAppointments = jobTable.select("count(Job_ID)","Date like '"+month+"%'");
            for (String[] i: billedAppointments){
                req.setAttribute("doneAppointments",i[0]);
                System.out.println(i[0]);
            }

            ArrayList<String[]> ongoingAppointments = jobTable.select("count(Job_ID)","Status in ('Done','Ongoing') and Date like '"+month+"%'");
            for (String[] i : ongoingAppointments){
                req.setAttribute("ongoingAppointments",i[0]);
            }

            ArrayList<String[]> totalIncome = jobTable.select("sum(Total)","Status in ('Billed') and Date like '"+month+"%'");
            ...
            req.setAttribute("branchId","Overall Report");
