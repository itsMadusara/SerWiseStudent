...
public class doneJobList extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            HttpSession session = req.getSession();
            String branchId = (String) session.getAttribute("branchId");
            ArrayList<String[]> results = jobTable.select("*","Status='Done' AND Branch_Id="+branchId);
            RequestDispatcher requestDispatcher = req.getRequestDispatcher("/Cashier/Appointments/AppointmentHome.jsp");
            int noOfRows = 0;
