  ...
  <div>
    ...
    <div class="two-content-div-form">
      ...
      <form action="../../ServletAddLubeAppointment" method="post">
        ...
        <label for="Service">Select Sevices:</label>
        <select name="Service_Id" id="Service" required>
          <%
            try{
              connection = DriverManager.getConnection(connectionUrl+database, userid, password);
              statement=connection.createStatement();
              PreparedStatement ps1=connection.prepareStatement("select * from serwise.service where Description=?");
              ps1.setString(1,"Lube");
              resultSet = ps1.executeQuery();
              while(resultSet.next()){
          %>
          <option value="<%=resultSet.getString("Service_Id")%>"><%=resultSet.getString("Name")%></option>
          <%
              }
              connection.close();
            } catch (Exception e) {
